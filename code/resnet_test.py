...
def resnet_1():
    ...
    filepath = "resnet_1.hdf5"
    model_checkpoint = ModelCheckpoint(filepath, monitor='val_loss', verbose=1, save_best_only=True, period=2)
    history = model.fit(x_train, y_train_hot, epochs=8, batch_size=10, shuffle=True, validation_data=(x_test, y_test_hot), callbacks=[model_checkpoint],)
    plt.figure()
    ...
    plt.figure(figsize=(30, 10))  # 这里只看20张，实际上并不需要可视化图片这一步骤
    for i in range(30):
